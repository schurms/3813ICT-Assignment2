...
4. Additional Features Implemented / Not Implemented
...
### 2. APPLICATION START COMMANDS
- Start Mongodb - Terminal Window 1 (This starts the mongodb server) CD to mongodb bin directory
```javascript
./mongod
```
...
```
### 3. KEY DESIGN ASSUMPTIONS
- Data is loaded from Mongodb on the server on an as required basis.  It was decided not to load all the data into the client as when multiple people were working on the data, then concurrent update issues can occur.  Thus the source of truth for all data is the server.
- Security
...
- Data persistence – All data (User/Group/Channel/Messages) is persisted within Mongodb.  Data can be modified, and it is retained from session to session.
...
### 4. FEATURES NOT IMPLEMENTED (These assignment features were not implemented)
- Display Message on user leaving channel
